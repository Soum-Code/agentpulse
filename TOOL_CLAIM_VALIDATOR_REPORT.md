...
**That follow-on has now been run, against an external corpus, and the result changes how this report should be read — see `TOOL_CLAIM_EXTERNAL_TEST_REPORT.md`.** On 8,353 spans of real agent output spanning 3 benchmarks, 4 harnesses and 5 models, the validator extracted **zero** claims. Not a low score — nothing at all. The cause is a design-premise mismatch rather than a tuning gap: `TOOL_PATTERNS` requires the agent to *narrate* tool use in prose ("I used the X tool"), but in structured-tool-calling harnesses invocation is a `tool_call` field and the prose narrates intent instead.

The figures above remain a correct measurement of what they measured. What they do not establish is applicability: every one of the 19 cases was hand-written in the phrasing the regex expects, so the benchmark could not have surfaced this. The §3 note that the validator "misses paraphrased claims" understates it — on these agents there is nothing to paraphrase, because tool use is never described in text.
